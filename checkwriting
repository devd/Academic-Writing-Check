...
if ($vimmode) {
  ($passive_color,$typography_color,$weasel_color,$dup_color,$abbr_color,$strunk_color,$user_color,$def_color) = (
    '**',
    '**',
    '**',
    '**',
...
print $def_color unless $vimmode;
my $filename="";
my @files=@ARGV;
$options{'diffmode'}=0;
if(not @files){
...
while(($filename=shift @files)){
    ...
    #remove lstlistings from the check
    #TODO: remove lstlistings, but replace each line with a blank line
    # so that line numbers don't get screwed up
    #$str =~ s/\\begin\{lstlisting\}.*?\\end\{lstlisting\}//sg;
    ...
    if($filename =~ m/bbl$/){
      ...
      $count{'typography'} += fixURL() if $options{'typography'};
    }else{
      #TODO there is a much better way to do this dispatch
      #by keeping the function refs keyed by the same key as
      #options and count.
      $count{'typography'} = fixTypography() if $options{'typography'};
      $count{'passive'} = fixPassive() if $options{'passive'};
      $count{'dups'} = fixDups() if $options{'dups'};
      $count{'weasel'} = fixWeasel() if $options{'weasel'};
      $count{'strunk'} = fixStrunk() if $options{'strunk'};
      $count{'abbr'} = fixAbbr() if $options{'abbr'};
      $count{'user'} = fixUserWords() if $options{'user'} and @user_words;
    }
    next unless sum(values %count);
    my $linenum=0;
    if($filename =~ m/([^\/]*)$/){
        $filename = $1 ;
    }
    my $extra="";
    utf8::encode($str);
    open my $fh,'<:encoding(utf8)',\$str or die $!;
    while(my $line= <$fh>){
      $linenum++;
    ...
    unless ($vimmode) {
      print "\n";
      print $passive_color,$count{'passive'}||0,' PASSIVE VOICE',"   " if $options{'passive'};
      print $typography_color,$count{'typography'}||0,' TYPOGRAPHY ERRORS',"   " if $options{'typography'};
      print $weasel_color,$count{'weasel'}||0,' WEASEL WORDS',"   " if $options{'weasel'};
      print $dup_color,$count{'dups'}||0,' DUPLICATES',"   " if $options{'dups'};
      print $strunk_color,$count{'strunk'}||0,' STRUNK AND WHITE',"   " if $options{'strunk'};
      print $abbr_color,$count{'abbr'}||0,' ABBR',"   " if $options{'abbr'};
      print $user_color,$count{'user'}||0,' USER' if $options{'user'};
      print $def_color,"\n";
    }
...
"written");
  ...
  return $str =~ s/$regex/\x{EFFF}$def_color$passive_color$1$def_color$2$passive_color$3$def_color\x{EFFF}/goi;
}
...
sub fixTypography{
  ...
  $c += ($str =~ s/([^\s]*[A-Z]\.)(?=\s|$)/\x{EFFF}$typography_color$1<add a \\\@>$def_color\x{EFFF}/og);
...
| finalize | importantly | insightful | in[ ]terms[ ]of | interesting(?:ly)? | irregardless | meaningful(?:ly)? | nice(?:ly)? |
